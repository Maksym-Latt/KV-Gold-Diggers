...
fun GameScreen(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        HoneyOverlay(expansion = honeyExpansion, wavePhase = wavePhase)
...
fun HoneyOverlay(expansion: Float, wavePhase: Float) {
    Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        val honeyHeight = height * expansion

        val honeyColor = Color(0xffebb236)
        val honeyEdgeColor = Color(0xffc9922f) // Darker honey color for the edge

        // Draw shadow/edge path first
        val edgeThickness = 12.dp.toPx()

        fun drawHoneyPath(h: Float, phase: Float, isEdge: Boolean) {
            val path =
                androidx.compose.ui.graphics.Path().apply {
                    ...
                    drips.forEachIndexed { i, (wRatio, dMult) ->
                        ...
                        if (expansion < 0.95f) {
                            ...
                            val waveDepth =
                                (30.dp.toPx() * dMult) +
                                        (kotlin.math.sin(phase + phaseOffset) *
                                                12.dp.toPx())
            ...
            drawPath(path = path, color = if (isEdge) honeyEdgeColor else honeyColor)
        }

        // Draw main body and then the edge slightly lower
        drawHoneyPath(honeyHeight, wavePhase, false)
        if (expansion < 0.98f) {
            drawHoneyPath(honeyHeight + edgeThickness, wavePhase, true)
            // Redraw main body over edge to keep only bottom visible
            drawHoneyPath(honeyHeight, wavePhase, false)
        }
    }
}
